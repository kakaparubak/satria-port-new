  ...
  useGSAP(() => {
    ...
    timelineTop
      .fromTo(
        topRole.current,
        { opacity: "0%", y: 0 },
        { opacity: "100%", y: 40, duration: 1, ease: "power4.inOut" }
      )
      .fromTo(
        topRole.current,
        { opacity: "100%", y: 40 },
        {
          ...
          duration: 1,
          ease: "power4.inOut",
          onComplete: setIndex2,
          onCompleteParams: [(i: number) => (i + 2) % roles.length],
        },
        "+=1"
      );
    timelineBottom
      ...
      .fromTo(
        bottomRole.current,
        { opacity: "0%", y: -40 },
        { opacity: "100%", y: 0, duration: 1, ease: "power4.inOut" },
        "+=1"
      );
  });
  ...
  return (
    <div className="relative md:left-6 lg:left-2.5 flex flex-col md:flex-row lg:flex-row md:gap-2.5 lg:gap-2.5 mt-8 justify-center items-center">
      ...
      <div className="md:w-[320px] lg:w-[320px] relative md:-top-5.5 lg:-top-5.5 text-center changing-text flex flex-col justify-center md:items-baseline lg:items-baseline items-center underline underline-offset-4 decoration-2 font-covgrace tracking-wider text-indigo-400 text-4xl">
        <p id="top-role" className="opacity-0" ref={topRole}>
          {roles[index2]}
        </p>
        <p id="bottom-role" className="opacity-50" ref={bottomRole}>
          {roles[index1]}
        </p>
      </div>
    </div>
